...
//   Serial.print("Robo"); Serial.print(" ");Serial.print(a); Serial.print(" "); Serial.println(b);    //prints the y coordinate read from python back to python
  ...
  while(abs(y_diff) - distTravel > 0 && y_diff > 0){
    distTravel = distTrav();
    Serial.print("Foward"); Serial.print("\t travel distance: "); Serial.println(distTravel);
    forwardSpeed = 17.0;
    angularVelocity = 0.0;
    speedControl();
  }
  ...
  while(abs(y_diff) - distTravel > 0 && y_diff < 0){
    distTravel = distTrav();
    Serial.print("Backward"); Serial.print("\t travel distance: "); Serial.println(distTravel);
    forwardSpeed = -17.0;
    angularVelocity = 0.0;
    speedControl();
  }

  while(abs(abs(y_diff) - distTravel) < 0.05){
    ...
    while(x_diff < 0){
      ...
      delay(2000);
      resetDistance();
      Serial.print("travel distance: ");  Serial.println(distTravel);
        while(abs(x_diff) - distTravel > 0){
          distTravel = distTrav();
          Serial.print("Foward"); Serial.print("\t travel distance: "); Serial.println(distTravel);
          forwardSpeed = 17.0;
...
//  rightSetpoint = forwardSpeed + angularVelocity - 2.75;
//  motors.control(-(map(rightSetpoint, 0, 40, 0, 255)),-(map(leftSetpoint, 0, 40, 0, 255)));
//}
